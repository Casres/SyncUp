...
**Soft deletes** (Lead Manager Decision #2, locked 2026-05-02) — `User`, `Event`, `Friendship`, and `SocialGroup` carry a `deletedAt DateTime?` column. Delete endpoints set `deletedAt` and the repository layer filters `deletedAt IS NULL` from every read. Hard deletes still apply to ephemeral records (poll votes, notifications, session data). Cascade rules for any *hard* delete must be confirmed per entity before implementing the endpoint.

**Auth enforcement — service layer + Postgres RLS as defence in depth** (Lead Manager Decision #3, locked 2026-05-02). Clerk JWT is verified in `auth.middleware.ts` and the decoded user is attached to every Fastify request. All permission checks happen in service code — RLS is the backstop, not the primary enforcement. The middleware must wrap each request in a transaction and run `SET LOCAL app.current_user_id = '<User.id>'` before any query so RLS policies can resolve. The `current_app_user_id()` helper function is created by the `_rls_policies` migration. If the variable is unset the helper returns `NULL` and policies fail closed. See `prisma/HANDOFF.md` for the full runtime contract.

**Seed data** (Lead Manager Decision #4, locked 2026-05-02) — Extended seed: 3–5 users with varied profiles, friendships covering all label categories, past + upcoming + recurring events (with cancelled and rescheduled exceptions), one social group with members + polls + suggestions, varied availability patterns. Plus the special-case rows for testing edge states (no-availability friend, blocked viewer, co-host event, full RSVP spread, regular-member-not-admin, closed poll). The current `prisma/seed.ts` is the legacy minimal seed and is OUT OF DATE — it needs to be expanded to match Decision #4. **The seed file and the `prisma.seed` entry in `package.json` must be removed before any production deploy.**

**Migration strategy** (Lead Manager Decision #5) — `prisma migrate dev` locally, `prisma migrate deploy` in CI and Railway.

---

## Decisions Still Open (Do Not Guess — Ask Before Implementing)

- Cascade rules for any record type that is NOT on the soft-delete list (poll votes, notifications, session data, exceptions). Confirm with Lead Manager per entity before implementing the delete endpoint.
- Whether to introduce a separate `DATABASE_URL_APP` env var for the non-owner runtime role. Currently `.env` only declares `DATABASE_URL`. See `prisma/HANDOFF.md`.
