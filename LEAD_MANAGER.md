...
| Backend | EXPLORE Cache + Rate-Limit (Phase B) | **PARTIAL (2026-05-22, prompt redesigned 2026-05-23)** — `src/middleware/exploreRateLimit.ts` exists (72 lines, committed in `bb9a7e6`) AND **IS wired** in `src/routes/explore.routes.ts` as `preHandler: [exploreRateLimit]` on `GET /feed` (the prior tracker note "(d) not registered" was incorrect — verified via grep 2026-05-23). The remaining real deviations: (a) limit defaults to 30 req/hr from `env.EXPLORE_RATE_LIMIT` instead of the spec's 20 req/hr; (b) no 5-burst / 60s sub-budget; (c) emits `Retry-After` only — no `X-RateLimit-*` headers; (d) the cache is INLINE inside `explore.service.ts` (`safeRedisGet`/`safeRedisSet` helpers) instead of a separate typed module. **Outstanding work** (defined in `agent-prompts/EXPLORE_BACKEND_AGENT_PROMPT.md` Phase B section — REDESIGNED 2026-05-23 to fit the actual on-disk `ExploreCategory`/`ExploreSource`/`ExploreVenue` types instead of the abandoned `ExploreCachePort`/`ExploreEvent`/`ExplorePlace` model): extract inline cache into `src/services/exploreCache.ts` (preserve the existing `explore:feed:{lat2}:{lng2}:{category}` and `explore:detail:{id}` keys byte-for-byte to avoid cold-starting the cache), add burst budget + `X-RateLimit-*` headers to the middleware, lower `EXPLORE_RATE_LIMIT` default to 20, add `EXPLORE_RATE_LIMIT_BURST` (default 5) and `EXPLORE_CACHE_TTL_SECONDS` (default 600) env fields. No new route wiring needed. Handoff at `social-calendar-api/src/middleware/EXPLORE_CACHE_RATELIMIT_HANDOFF.md`. |
| Backend | EXPLORE Cron + Billing (Phase C) | **PENDING (depends on EXPLORE Phase A + B)** — prompt: `agent-prompts/EXPLORE_BACKEND_AGENT_PROMPT.md` (Phase C section). `node-cron` pre-warmer (default `0 */2 * * *`) calls `ExploreService.fetchFeed` for every city in `EXPLORE_PREWARM_CITIES` so cache is always warm; serialised per-city, production-only. GCP billing alerts via Terraform at $25 / $50 / $100 (notify-only — never auto-disable; Places-only filter, Eventbrite tracked separately). `exploreFeatured.hook.ts` identity stub injected into `ExploreService.featuredHook` as the wire-in point for the future B2B Featured listings agent. Handoff at `social-calendar-api/src/workers/EXPLORE_CRON_BILLING_HANDOFF.md`. |
...
| Frontend | Onboarding (R15-7..R15-13) | **PENDING** — prompt: `agent-prompts/ONBOARDING_AGENT_PROMPT.md`. Welcome + Sign-Up Steps 1–6 + Sign-In + Forgot-Password screens already on disk; builds the post-Step-6 tail (PushPermissionGate · FriendFind Decision/Matches/NoWorries · YoureIn), the R15-11 Friends-tab ContactsDeniedAffordance, and the R15-13 first-run empty-state copy gating across EmptyHome / EmptyFriends / EmptyGroups / inline EmptyNotifications. Adds a `useIsFirstRun()` hook over `useMe()` (derives from `User.stats.{hosted,friends,groups} === 0`, one-way + global). Step 6 "Continue"/"Skip" rewired to navigate `PushPermissionGate` (NOT direct to Tabs). `expo-notifications` + `expo-contacts` required. Handoff at `social-calendar-mobile/src/screens/auth/AUTH_ONBOARDING_HANDOFF.md`. |
| Frontend | AttendeesSheet R15 extension (R15-1..R15-6) | **PENDING** — prompt: `agent-prompts/ATTENDEES_SHEET_AGENT_PROMPT.md`. AttendeesSheet, AttendeeRow, QuickProfileSheet already on disk (R10/R11/R12-5/R12-6 era); this agent applies R15-1 (row-tap → QuickProfileSheet, terminal; self-row no-op), R15-2 (friend vs non-friend variants — friend variant adds status-ringed avatar + Friend Type chip replacing the CTA block; untyped friend omits chip entirely), R15-3 (RSVP grouping with HOSTS pinned, hosts double-appear; HOST chip on host row), R15-4 (header magnifier reveal + SearchInputBar — section overlines hidden during active search, flat alphabetical list, chips compose), R15-5 (chip filter bar sticky on scroll), R15-6 (offline state mirrors NotifSheet — OfflineBar + "SYNCED N AGO" sub-line, disabled "Invite more" + ⋯ menu, auto-cancel armed remove). Creates `src/components/social/SearchInputBar.tsx`. Wires SearchOverlay PEOPLE row body tap → QuickProfileSheet (non-friend variant) — removes the standing `// TODO(quick-profile-sheet)`. Handoff at `social-calendar-mobile/src/components/social/ATTENDEES_SHEET_R15_HANDOFF.md`. |
