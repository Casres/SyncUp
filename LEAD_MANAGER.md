...
| Search Overlay (GAP 2) | Screens complete | `src/components/search/` — full-screen overlay (R8-1..R8-7), sectioned results, recents (max 7), React Query cache integration |
...
| Seed Rebuild | Schema + all domain agents complete | `prisma/seed.ts` rewritten per Decision #4 — 5 users, friendships, co-host event, recurring event with exceptions, RSVP spread, closed poll. **Dev-only file — must be deleted before production.** |
| EXPLORE Gateway (Phase A) | Backend Cleanup + Socket.io complete | `/explore/*` route group, Eventbrite + Google Places client wrappers, unified response shapes (no client-direct upstream calls) |
| EXPLORE Cache + Rate-Limit (Phase B) | EXPLORE Gateway complete (contracts can be drafted in parallel) | Redis cache wrapper (TTL 10–15min) + per-user rate-limit preHandler (20/hr default) on `/explore/feed` |
| EXPLORE Cron + Billing (Phase C) | EXPLORE Gateway + Cache+Rate-Limit complete | `node-cron` pre-warmer for popular cities, GCP billing alerts at $25/$50/$100 (notify-only), Featured-listings hook stub for future B2B revenue layer |
...
| Backend | Seed Rebuild | **COMPLETE (2026-05-23)** — Shipped by the Step 3 Wave 1 Seed Rebuild agent (uncommitted, audited and certified during Step 3 recovery cleanup 2026-05-23). On disk: `prisma/seed.ts` (~650 lines) implements the full Decision #4 extended seed end-to-end — 5 users (alice/bob/carol/dan/eve, upsert by `seed_clerk_*`), 4 friendships + 6 labels (PENDING Eve→Alice has no labels by design), 1 AvailabilityBlock (dan blocks alice), 2 SocialGroups + 6 members, 4 Events + 5 EventOrganisers + 7 EventInvites (full RSVP spread on upcoming), 2 EventExceptions (CANCELLED + RESCHEDULED) on the recurring event, 1 closed GroupPoll + 3 PollOptions + 4 PollVotes, 1 EventSuggestion. All non-User rows use `seed-` prefix, deterministic IDs, `upsert` everywhere → idempotent. `tsc --noEmit` clean. Mandatory production-deletion header present verbatim. Standalone — no imports from `src/`. **Idempotency is schema-validated only — live DB run pending Postgres availability via DevOps `docker-compose`.** Handoff: `social-calendar-api/prisma/SEED_HANDOFF.md`. **Mandatory reminder: file + `package.json prisma.seed` entry MUST be deleted before any production deploy** — DevOps Railway Deploy `DEPLOY_CHECKLIST.md` already gates on this. |
| Backend | EXPLORE Gateway (Phase A) | **COMPLETE (2026-05-22)** — Shipped in commit `bb9a7e6` (mixed-scope "GAP 3 + monorepo catch-up" commit). On disk: `src/services/explore.service.ts` (wired) + `src/services/eventbrite.client.ts` + `src/services/googlePlaces.client.ts` + `src/controllers/explore.controller.ts` + `src/routes/explore.routes.ts` (registered in `app.ts` at `/explore` prefix) + `src/types/explore.types.ts` (with `ExploreCategory` / `ExploreSource` / `ExploreVenue` model). Behind Clerk auth via the global middleware. Tracker row was stale through 2026-05-23 — flipped during Step 3 recovery cleanup. **`agent-prompts/EXPLORE_BACKEND_AGENT_PROMPT.md` Phase A section is SUPERSEDED.** Architectural note: the existing implementation uses clients in `src/services/` (alongside the service), not in a separate `src/clients/` dir. The Step 3 Wave 1 Phase A agent wrote orphan parallel clients under `src/clients/` before hitting usage limit; orphans deleted during cleanup. If a future refactor wants to extract clients into their own dir, it should be a deliberate scoped task, not bundled into Phase B/C work. |
| Backend | EXPLORE Cache + Rate-Limit (Phase B) | **COMPLETE (2026-05-24)** — Extracted inline Redis helpers from `explore.service.ts` into `src/services/exploreCache.ts` (typed module: `getFeed/setFeed/getDetail/setDetail`; same key format, no cache cold-start). Fixed rate-limit deviations: lowered `EXPLORE_RATE_LIMIT` default 30→20; added `EXPLORE_RATE_LIMIT_BURST=5/60s` sub-budget; emits `X-RateLimit-Limit/Remaining/Reset` on every `/feed` response; `Retry-After` + `reason` field (`hour_quota`/`burst_quota`) on 429. Added `EXPLORE_CACHE_TTL_SECONDS=600` env var. `tsc --noEmit` clean. Handoff: `social-calendar-api/src/middleware/EXPLORE_CACHE_RATELIMIT_HANDOFF.md`. |
| Backend | EXPLORE Cron + Billing (Phase C) | **COMPLETE (2026-05-24)** — `node-cron` pre-warmer (`0 */2 * * *`, 6 cities, serialized) starts in production after `app.listen()`; calls `exploreService.getFeed(lat, lng, 'all', 0)` per city so writes flow through `exploreCache.setFeed`. `exploreFeaturedHook` identity stub in `src/workers/exploreFeatured.hook.ts` is called inside `exploreService.getFeed` after merge, before cache write — the B2B Featured agent owns the real implementation. Terraform at `src/infra/gcp-billing-alerts.tf`: 3 budgets ($25/$50/$100), notify-only, Places-only. Manual gcloud fallback in `gcp-billing-alerts.README.md`. `EXPLORE_PREWARM_CITIES`, `EXPLORE_PREWARM_CRON`, `GCP_*` env vars added. `node-cron@4.2.1` installed. `tsc --noEmit` clean. Handoff: `social-calendar-api/src/workers/EXPLORE_CRON_BILLING_HANDOFF.md`. |
| Frontend | Search Overlay (GAP 2) | **COMPLETE (2026-05-21)** — Shipped in commit `966e846` ("feat(mobile): GAP 2 — Search overlay (R8-1 through R8-7)"), on disk at `src/components/social/SearchOverlay.tsx` + `src/components/social/SearchContext.tsx` + `src/components/emptyStates/EmptySearch.tsx`. Four-section results FRIENDS → PEOPLE → GROUPS → EVENTS, recents capped at 7 via AsyncStorage, locked input bar, `staleTime: 5min` React Query cache, no Zustand. Wired into FlowHeader `right` slot on Home / Friends / Groups. PEOPLE-row body tap → QuickProfileSheet (non-friend variant) wire-up is the one remaining stub deferral and is now owned by `ATTENDEES_SHEET_AGENT_PROMPT.md` (R15-1 surface). **`agent-prompts/SEARCH_OVERLAY_AGENT_PROMPT.md` is SUPERSEDED** — file kept with a tombstone header for audit; do not spawn. Tracker row was stale through 2026-05-23 — flipped during the Step 3 recovery cleanup. |
| Frontend | GAP 6 — NotifSheet navigation wiring (R12-1) | **COMPLETE (2026-05-21)** — Verified the navigation graph against ANCHOR-DESIGN.txt before edits and found the wiring already in place in `src/components/notifications/NotifSheet.tsx`: `navEventDetail` → Tabs.HomeTab.EventDetail, `navFriendProfile` → Tabs.FriendsTab.FriendProfile, `navCreateEvent` → CreateEventModal.Step1 with `prefilledInviteeIds: [friendId]` (modal, no tab switch). RsvpCard / EventReminderCard / CoHostCard / InboundBroadcastCard "Plan something" / GroupActivityCard all fire `medium` haptic at tap, InboundBroadcastCard body fires `light` (locked exception), and `onRead(notif.id)` is called at the tap moment — sheet then `dismiss()`es concurrent with the destination mount via the global `navigationRef`. FriendRequestCard / GroupInviteCard / CoHostRevokedCard correctly render NO nav affordance. `GroupActivityCard` routes via the existing hidden `GroupsTab` route (Friends / Groups SegmentedSwitcher not yet unified — pre-existing TODO comment retained for the future migration). No code changes needed for this gap. `tsc --noEmit` clean. |
| Frontend | GAP 7 — NotifSheet velocity-first gesture (R13-1) | **COMPLETE (2026-05-21)** — Phase A (HomeScreen pull-from-home) and Phase B (NotifSheet grab-handle pan) already implemented the velocity-first / displacement-fallback model; this gap added the two remaining spec items. (1) **Backdrop snap under reduced-motion** — `NotifSheetContext.animateTo()` now owns `backdropOpacity` as a provider-level `SharedValue` and switches it instantly at release on the reduced-motion path (200ms easeOut translate, instant backdrop). On the standard path, backdrop crossfades 280ms easeStd in lockstep with the spring. `NotifSheet.backdropStyle` consumes the driven value directly — no `interpolate` from `translateY`. (2) **Snap haptic** — fires at the snap-settle moment, not at pointer-up: `animateTo` now accepts an optional `onSettled` callback that the `openPeek` / `openFull` / `dismiss` helpers thread through to `withSpring` / `withTiming`'s completion callback (via `runOnJS`). HomeScreen's `snapPhaseA` and NotifSheet's `snapTo` both pass `() => fire('light')` so the haptic lands when the sheet visually arrives at the detent. Non-gesture call sites (bell tap, backdrop tap) keep their tap-time haptic and pass no `onSettled`. `tsc --noEmit` clean. |
| Frontend | GAP 8 — NotifSheet offline state (R13-2) | **COMPLETE (2026-05-21)** — UI surface (OfflineBar inline below sheet header + "SYNCED N AGO" mono sub-line via `formatRelative(lastSyncedAt)`, stale feed visible, network-dependent CTAs disabled at opacity 0.4 with `pointerEvents='none'` on FriendRequestCard / GroupInviteCard / InboundBroadcastCard "Plan something") was already in `src/components/notifications/NotifSheet.tsx` and the three card files. This gap added the **offline → online auto-refresh** missing from R13-2: a `useEffect` in NotifSheet tracks the previous `offline` value via `useRef` and, on the offline → online transition, calls `queryClient.invalidateQueries({ queryKey: queryKeys.notifications.all() })`. React Query's revalidation drives the new-items StaggerList entrance per spec. Local actions (Mark all read, Clear all, swipe Mute / Dismiss) continue to fire while offline (they're local mutations). OfflineBar's `aria-live="polite"` is already in `polish/OfflineBar.tsx`. `tsc --noEmit` clean. |
| Frontend | GAP 9 — BroadcastToast queued prop (R13-3) | **COMPLETE (2026-05-21)** — Verified `src/components/profile/BroadcastToast.tsx` already exposes `queued?: boolean` (default false). When `queued=true` the title swaps to `"Broadcast sent · was queued"` and the Undo pill is omitted; close X, auto-dismiss (3200ms), success haptic, `role="alert"` + `aria-live="polite"`, position, sizing, tokens, and border all unchanged. BroadcastSettingsScreen's preview-toast call site continues to use the default standard surface (queued not passed). No code changes needed. `tsc --noEmit` clean. |
| Frontend | GAP 10 — AudiencePickerSheet zero-friend state (R13-4) | **COMPLETE (2026-05-21)** — The presentational `src/components/profile/AudiencePickerSheet.tsx` already rendered the inline empty state (56×56 bgSunken tile + users glyph + "No friends yet" + "Add friends to invite them to events." + NO CTA) and disabled the Done pill when `mode === 'friends' && friends.length === 0`. The route wrapper `src/screens/profile/AudiencePickerSheetScreen.tsx` was short-circuiting to `<EmptyFriends />` instead of mounting the sheet, which hid the in-sheet empty state and the disabled Done pill from the user. This gap removed that short-circuit: the friends-empty case now falls through to mounting `<AudiencePickerSheet visible mode='friends' friends={[]} />` so the spec'd empty state, disabled Done, and active Close X all render per R13-4. `mode='types'` empty still uses the screen-level EmptyStateBlock (types empty is not covered by R13-4). `tsc --noEmit` clean. |
| Frontend | Cleanup — `src/mocks/` seed evacuation (CLAUDE.md) | **COMPLETE (2026-05-21)** — All 8 prior seed-data files (`users.ts`, `friendships.ts`, `friendTypes.ts`, `events.ts`, `groups.ts`, `availability.ts`, `notifications.ts`, `explore.ts`) deleted from the working tree. Replaced by a single tombstone `index.ts` that re-exports the same symbol names as empty arrays / `{}` records / minimal-shape empty shells (MOCK_ME, MOCK_EVENT_*, MOCK_GROUP_*, MOCK_POLL_*) so the 17 active consumers (9 in `src/api/*` and 8 in screens / components) still compile. Zero seed data, zero PII, zero hard-coded copy ships from this directory now — the app renders its zero-data empty states everywhere. The `EventOrganiser` / `EventOrganiserRole` / `EventRecurrence` / `EventExceptionEntry` type exports plus the `isoOffset()` date-math helper are preserved as stable contract surface. **Full directory removal is now gated only on each `src/api/*.ts` stub being rewired to a real backend endpoint** — once every consumer is rewired, delete `index.ts` and the dir disappears. Tombstone banner + remaining-consumer list lives at `social-calendar-mobile/src/mocks/MOCKS_HANDOFF.md`. **Note for production gate:** the existing Prisma `prisma/seed.ts` rule (Decision #4) is separate and still tracked under Backend → Seed Rebuild. `tsc --noEmit` clean. |
| Frontend | Cleanup — TweaksPanel (R14-1) absence audit | **COMPLETE (2026-05-21)** — Verified zero `TweaksPanel` references anywhere under `social-calendar-mobile/` (excluding `node_modules`). The component only appears in HTML prototype files and design docs at the monorepo root (`Reduced-Motion.html`, `EXECUTE-GAPS.md`, etc.), per the spec rule that TweaksPanel is prototype-only and must never ship in production. |
| Frontend | Onboarding (R15-7..R15-13) | **COMPLETE (2026-05-24)** — 5 post-Step-6 screens built (PushPermissionGate, FriendFindDecision, FriendFindMatches, FriendFindNoWorries, YoureIn); all registered in AuthNavigator with `gestureEnabled:false`; PushPermissionGate uses `animation:'none'`. `useIsFirstRun()` hook reads `useMyProfile().data.stats` — all three counters zero. `useContactsPermissionStatus()` one-shot OS query. `useContactsMatches()` React Query wrapper. `ContactsDeniedAffordance` component wired into FriendsListScreen below filter chips. `EmptyHome`, `EmptyFriends`, `EmptyGroups`, `EmptyNotifications` (inline in NotifSheet) all accept `firstRun` prop with locked R15-13 copy. HomeScreen, FriendsListScreen, GroupsListScreen, NotifSheet all wire `useIsFirstRun()`. SignUpStep6 `finishOnboarding()` now navigates to `PushPermissionGate`. `api/onboarding.ts` + `api/queryKeys.ts` updated. `tsc --noEmit` clean. Handoff: `social-calendar-mobile/src/screens/auth/AUTH_ONBOARDING_HANDOFF.md`. |
| Backend | Messaging Domain (R18) | **DONE + MERGED to `main` (2026-06-04, PR #1 `a62668a`)** — `Conversation` / `Message` / `ConversationParticipant` models + `ConversationType` enum; migration `20260603000001_messaging`. Repo/service/routes/controller mounted at `/conversations` (inbox, infinite thread, get-or-create DM gated on accepted-friendship + block, send, mark-read) + `POST /events/:id/chat` (organiser-only event-chat enable). Group chat auto-created as a side effect of `friendGroups.service` create/addMember (D4); event chat host-enabled, seeds organisers + invitees. `chat.socket.ts` (participant-gated join/leave + `chat:typing` relay), emits `chat:message:new`/`chat:conversation:new`; `eventChatArchival.worker.ts` (one-time event chats → `archivedAt = endsAt+48h`). **RLS:** ALL messaging writes route through the migration-owner `prisma` client (bypass RLS so INSERT…RETURNING never re-evaluates a SELECT policy — the `5f30e3a` class of bug); reads use inline-EXISTS SELECT policies; `ConversationParticipant` SELECT is own-rows-only (a co-participant policy self-recurses) with co-participant hydration via gated `…Owner` repo methods. `scripts/messaging-roundtrip.sh` written. `tsc`+`npm run build` green. **Verified:** migrate-deploy applied + round-trip runs **31/31, 0 fail, re-runnable** (`MESSAGING_ROUNDTRIP_RESULTS.md`). The run found+fixed a real group-chat auto-create FK bug — the FriendGroup was written in the uncommitted per-request app-client tx while the chat insert ran on a separate owner-client connection (FK `linkedGroupId` → no visible target → P2003, swallowed); fix `friendGroupsRepository.createWithGroupChatOwner` (atomic owner-client `$transaction`). CI Test job was pre-existing red on `main` (ci.yml omitted `CLOUDINARY_*` env that `env.ts` requires) — fixed via GitHub repo secrets wired into the Test job env. See `R18-PLAN.md` "Build notes". |
| Frontend | Messaging UI (R18) | **DONE + MERGED to `main` (2026-06-04, PR #1 `a62668a`)** — `src/api/conversations.ts`/`.types.ts` React Query hooks (useInbox · useThread infinite · useSendMessage · useGetOrCreateDirect · useEnableEventChat · useMarkRead) + query keys + barrel export. `src/components/messaging/*` (MessageBubble · TypingDots · ConversationAvatar · InboxRow · MessageInput · ThreadHeader · ChatThreadView + `format.ts`) + `emptyStates/EmptyMessages`. Screens: MessagesScreen (inbox), MessageThreadScreen (FriendsStack), EventChatScreen (HomeStack); routes + prop types wired. DM promoted from the R16-9 stub → real (R17-9, `FriendProfileScreen` DM button → `useGetOrCreateDirect` → MessageThread). NotifSheet M4 routing: message cards route straight to the thread (medium haptic) via a `conversationId`/`conversationType` payload hint. `tsc --noEmit` green. **Follow-ups since merge:** realtime socket client ✅ BUILT (`src/realtime/`, first socket.io-client on mobile); R17-1 Friends·Groups·Messages carousel ✅ BUILT on branch `r17-friends-carousel` (Groups + inbox now segments, not routes). **Pending:** device QA; socket not yet run against a live socket server. See `R18-PLAN.md` "Build notes". |
| Frontend | AttendeesSheet R15 extension (R15-1..R15-6) | **COMPLETE (2026-05-24)** — R15-1: QuickProfileSheet mounted inside AttendeesSheet; row tap sets `quickProfileTargetId`; self-row tap guard (no haptic, no nav). R15-2: `QuickProfileSheet` adds `isFriend` + `friendTypeName` props; friend variant shows status ring + FriendType chip; non-friend variant unchanged. R15-3: FlatList data is `ListItem[]` union; HOSTS pinned first then GOING→MAYBE→NOT GOING→NO RESPONSE; hosts double-appear; `keyExtractor` uses `section-id` composite; `AttendeeRow` gains `isHost` prop + HOST chip (accentSoft/accent trailing). R15-4: magnifier tap collapses title (200ms easeStd) + reveals SearchInputBar; "Cancel" exits search; query composes with chip filter; flat alphabetical list during active search; section overlines hidden. R15-5: chip rows rendered outside FlatList (inherently sticky). R15-6: `offline`+`lastSyncedAt` props; OfflineBar above chips; "SYNCED N AGO" sub-line; Invite more hidden; armed row auto-cancels. SearchInputBar created as standalone component. SearchOverlay PEOPLE→QuickProfileSheet was already wired (verified). `tsc --noEmit` clean. Handoff: `social-calendar-mobile/src/components/social/ATTENDEES_SHEET_R15_HANDOFF.md`. |
